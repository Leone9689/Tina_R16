...
.config: ./scripts/config/conf $(if $(CONFIG_HAVE_DOT_CONFIG),,prepare-tmpinfo)
	@+if [ \! -e .config ] || ! grep CONFIG_HAVE_DOT_CONFIG .config >/dev/null; then \
		...
		elif [ "x$(TARGET_BUILD_VARIANT)" = "xdragonboard" ]; then \
			[ -e target/allwinner/$(TARGET_BOARD)/defconfig-db ] && \
			cp target/allwinner/$(TARGET_BOARD)/defconfig-db .config; \
		fi \
	elif [ -e .config -a "x$(TARGET_BUILD_VARIANT)" = "xtina" ]; then \
		cp target/allwinner/$(TARGET_BOARD)/defconfig .config; \
	elif [ -e .config -a "x$(TARGET_BUILD_VARIANT)" = "xdragonboard" ]; then \
		cp target/allwinner/$(TARGET_BOARD)/defconfig-db .config; \
	fi
...
menuconfig: scripts/config/mconf prepare-tmpinfo FORCE
	if [ -z $(TARGET_BOARD) ]; then \
		...
		[ -e target/allwinner/$(TARGET_BOARD)/defconfig ] && \
		cp target/allwinner/$(TARGET_BOARD)/defconfig .config; \
		[ "x$(TARGET_BUILD_VARIANT)" = "xdragonboard" ] && \
		[ -e target/allwinner/$(TARGET_BOARD)/defconfig-db ] && \
		cp target/allwinner/$(TARGET_BOARD)/defconfig-db .config; \
		$< Config.in; \
		if cat .config|grep CONFIG_TARGET_BOARD=\"$(TARGET_BOARD)\" >> /dev/null; then \
			...
			[ "x$(TARGET_BUILD_VARIANT)" = "xdragonboard" ] && \
			cp .config target/allwinner/$(TARGET_BOARD)/defconfig-db; \
			echo ; \
		else \
			rm .config; \
		fi \
